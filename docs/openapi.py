...
def parse_function_desc(function_name, function_desc) -> dict:
    ...
    if route_function_args["query_parameters"] is not None:
        query_params_schema = create_model(route_function_args["query_parameters"]).schema(
    ...
    if route_function_args["path_parameters"] is not None:
        path_parameters_schema = create_model(route_function_args["path_parameters"]).schema(
            ref_template="#/components/schemas/{model}"
        )
    ...
    if route_function_args["response_model"] is not None:
        if get_origin(route_function_args["response_model"]) is list:
            list_item_schema = get_args(route_function_args["response_model"])[0].schema(
                ref_template="#/components/schemas/{model}"
            )
...
def get_openapi_schema():
    ...
    paths, schema_components = generate_openapi_paths_and_components(parsed_route_configs)
    schema = dict(
        openapi="3.0.2",
